...
public class P_2798_2 {
  ...
  public static void main(String[] args) {
    ...
    int temp = reflexive(arr, new boolean[N], 0, 3, -1, 0, M);
  ...
  public static int reflexive(int[] options, boolean[] root, int current, int max,
      int problem, int solution, int opt) {
    if (current == max) {
      if (solution > opt) {
        return problem;
      }
    ...
    for (int i = 0; i < options.length; i++) {
      ...
      problem = Math.max(problem,
          reflexive(options, root, current + 1, max, problem, solution + options[i], opt));
      root[i] = false;
    }
